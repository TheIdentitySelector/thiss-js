...
for f in `find . -printf '%P\n'`; do
   if [ "x$f" != "x" -a -f $f ]; then
      ...
      envsubst '${BASE_URL} ${STORAGE_DOMAIN} ${MDQ_URL} ${SEARCH_URL} ${DEFAULT_CONTEXT} ${LOGLEVEL} ${WHITELIST} ${MIN_SEARCH_LENGTH} ${SAA_COMPLIANT_BROWSERS}' < $f > /usr/share/nginx/html/$f
   fi
done
